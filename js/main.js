  ...
  filterTags.forEach((tag) => {
    tag.addEventListener('click', function () {
      ...
      courses.forEach((course) => {
        if (
          filter === 'all' ||
          course.getAttribute('data-category') === filter
        ) {
...
function setupSorting() {
  ...
  sortSelect.addEventListener('change', function () {
    ...
    const fabricacionCourses = Array.from(
      document.getElementById('fabricacion-cursos').children
    );
    const transformacionCourses = Array.from(
      document.getElementById('transformacion-cursos').children
    );
    const solarpunkCourses = Array.from(
      document.getElementById('solarpunk-cursos').children
    );
  ...
  courses.sort((a, b) => {
    if (sortBy === 'number') {
      ...
      const durationA = parseInt(
        a.querySelector('span.bg-blue-100').textContent
      );
      const durationB = parseInt(
        b.querySelector('span.bg-blue-100').textContent
      );
